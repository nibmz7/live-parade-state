...
export default abstract class BaseDepartment extends LitElement {
  ...
  abstract onUserSelected(user: User): void;
    ...
    const userTemplate = (user: User) => {
      ...
      return html`
        <div
          ...
          @click="${onPressed(() => this.onUserSelected(user))}"
        >
          <slot name="full-name">user.full</slot>
